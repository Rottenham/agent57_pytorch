...
def get_preprocess_func(env_name):
    ...
    if "Breakout" in env_name:
        ...
        raise NotImplementedError(f"Frame processor not implemented for {env_name}")
...
def segments2contents(segments, burnin_len, is_grad=False, device=torch.device("cpu")):
    ...
    states = torch.stack([torch.tensor(np.vstack(seg.states), requires_grad=is_grad)
                         for seg in segments], dim=1).float().to(device)

    # (burnin_len+unroll_len, batch_size)
    actions = torch.stack([torch.tensor(seg.actions) for seg in segments], dim=1).to(device)

    # (burnin_len+unroll_len, batch_size)
    ex_rewards = torch.stack([torch.tensor(seg.ex_rewards, requires_grad=is_grad)
                             for seg in segments], dim=1).float().to(device)

    # (burnin_len+unroll_len, batch_size)
    in_rewards = torch.stack([torch.tensor(seg.in_rewards, requires_grad=is_grad)
                             for seg in segments], dim=1).float().to(device)
    ...
    last_state = torch.stack([torch.tensor(np.vstack(seg.last_state), requires_grad=is_grad)
                             for seg in segments], dim=0).float().to(device)
